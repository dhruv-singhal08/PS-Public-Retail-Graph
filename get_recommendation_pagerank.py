...
prod_attributes = [ 'colour_group_name',
...
featur_weight = { 'graphical_appearance_name' : 1.319941e-01,
...
def get_recommendation(root):
    ...
    #finding the most likely product to be recommended and its matching node count
    prod_dict, prod_score = neighbour_attr_node_match(product_set, node_neighbourbood, x_label, x_value)
    prod_score_sorted = sorted( prod_score.items(), key=lambda kv:
                 (kv[1], kv[0]),reverse=True)
    ...
    #obtaining the attributes for all product to be recommeded
    product_attributes_values = product_attributes_dict(recomd_prod_list)
    
    #recommedning some product based on sales 
    recomd_prod_score = {}
    for i in recomd_prod_list:
        recomd_prod_score[i] = prod_score_dict[i]
    
    recomd_prod_score = sorted( recomd_prod_score.items(), key=lambda kv:(kv[1], kv[0]),reverse=True)
    ...
    #obtaining product score based on attribute value
    ...
    #recommedning top 6 most likely product
